...
"""
    This class instantiates an object that is responsible for communication with the drone.
    It establishes a connection to the drone using sockets, sends to the drone a commands from the app
    and sends back responses received from the drone.      
"""
...
class DroneManager(metaclass=Singleton):
    ...
    def patrol(self):  # sequentially moves up, rotates 90 degrees clockwise and go moves down repeatedly until  a "stop petrol" command is sent
    ...
    def video_jpeg_generator(self):  # Streaming from the drone's camera
        for frame in self.video_binary_generator():
            if self._is_enable_face_detect:
                """
                    Enabling face detection from the drone's camera using OpenCv library
                    and haar cascade classifier. Once a face are detected it will seen on the streaming marked with rectangle.   
                """
                if self.is_patrol:
                    self.stop_patrol()
            ...
            if self.is_snapshot:  # taking a snapshot from the camera
